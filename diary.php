<?php
use Carbon\Carbon;
use Cartazfacil\IntegracaoBluesoft\Bluesoft;
use IntegracaoIquegami\General;
...
function iterateProducts($index, $count)
{
    ...
    //Data dia anterior
    $dia = Carbon::now();
    $time = $dia->subHour()->format('d/m/Y H:m');
    ...
    file_put_contents('./diary.txt', (string) $index);
...
function iteratePrices($index, $count)
{
    ...
    $bluesoft->getPrices($index, $count, null, ['dataAlteracaoInicio' => $time]);
    ...
    //Salvar posição em arquivo de status
    file_put_contents('./diary-price.txt', (string) $index);

    //Cálculo de páginação chunks (quantidade por quantidade)
    //$index = $count * (($index + $count) / $count);
...
$prod_saved_index = (int) file_get_contents('./diary.txt');
...
file_put_contents('./diary.txt', (string) 0);

$price_saved_index = (int) file_get_contents('./diary-price.txt');
...
//Resetar posição para inicio em arquivo de status
file_put_contents('./diary-price.txt', (string) 0);

echo "\ndiary.php: Execução de script finalizado!";